<?php 
	...
	if(isset($price_discount['min_days']) && !empty($price_discount['min_days'])){
		$min_days = $price_discount['min_days'];
	}else{
		$min_days = '';
	}

	if(isset($price_discount['max_days']) && !empty($price_discount['max_days'])){
		$max_days = $price_discount['max_days'];
	}else{
		$max_days = '';
	}	

	if(isset($price_discount['discount_type']) && !empty($price_discount['discount_type'])){
		$discount_type = $price_discount['discount_type'];
	}else{
		$discount_type = '';
	}

	if(isset($price_discount['discount_amount']) && !empty($price_discount['discount_amount'])){
		$discount_amount = $price_discount['discount_amount'];
	}else{
		$discount_amount = '';
	}
...
<div class="price_discount_group redq-remove-rows sort ui-state-default" style="background: none; border: none;">
	...
	<div class="redq-show-bar">
		<h4 class="redq-headings"> <?php _e('Days','redq-rental'); ?> ( <?php echo esc_attr($min_days); ?> - <?php echo esc_attr($max_days); ?> ) <?php _e('-  Discount','redq-rental'); ?> : <?php if($discount_type != 'percentage'): ?> <?php echo esc_attr($discount_amount); ?><?php echo esc_attr(get_woocommerce_currency_symbol()); ?> <?php else: ?><?php echo esc_attr($discount_amount); ?>% <?php endif; ?>
